...
- 2026-04-01: Completed AI Explain Code — explainCode server action with auth/Pro gating/Zod validation/rate limiting, Explain button (Sparkles) in CodeEditor macOS header next to Copy, Code/Explain tab toggle after explanation generated, markdown rendering in same editor container, Crown icon + tooltip for free users, isPro/title/showExplain props threaded through ItemDrawer→ItemDrawerContent→CodeEditor, ShadCN tooltip component added, 10 unit tests
- 2026-04-01: Completed AI Prompt Optimization — optimizePrompt server action with auth/Pro gating/Zod validation/rate limiting, Optimize button (Sparkles) in MarkdownEditor header for prompt type only (edit/create mode), Write/Preview/Optimized tab toggle after result, "Use this" replaces editor content, Crown icon + tooltip for free users, isPro/title/showOptimize threaded through ItemDrawerEdit→ItemTypeFields→MarkdownEditor and NewItemDialog→ItemTypeFields→MarkdownEditor, 10 unit tests
