...
export function UpgradePricing({ monthlyPriceId, yearlyPriceId }: UpgradePricingProps) {
  const [isYearly, setIsYearly] = useState(false);
  const [loading, setLoading] = useState(false);

  const proPrice = isYearly ? "£6" : "£8";
  const ctaLabel = isYearly ? "Upgrade — £72/year" : "Upgrade — £8/month";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="border-blue-500/40 bg-blue-500/5 relative">
          <div className="absolute -top-3 left-1/2 -translate-x-1/2">
            <Badge className="bg-blue-500 text-white border-0">Recommended</Badge>
          </div>
          <CardHeader className="pb-4">
